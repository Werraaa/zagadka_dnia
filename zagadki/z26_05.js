// --- 1. STYLE CSS (Wstrzykiwane dynamicznie do dokumentu) ---
const styles = `
    :root {
        --grid-size: 4;
        --cell-size: 80px;
...
// Wstrzykiwanie styli do sekcji <head> dokumentu głównego
const styleSheet = document.createElement("style");
styleSheet.innerText = styles;
document.head.appendChild(styleSheet);


// --- 2. STRUKTURA HTML (Generowana dynamicznie przez JS) ---
// Załóżmy, że ładujesz to do jakiegoś kontenera na stronie głównej (np. o id "puzzle-content")
// Jeśli funkcja ma się wykonywać w konkretnym miejscu, możesz zmienić `document.body` na swój kontener.
const targetContainer = document.getElementById("puzzle-content") || document.body;
...
`;
targetContainer.appendChild(gameWrapper);
...
// Ścieżka relatywna wychodząca z folderu "zagadki" poziom wyżej do obrazków
    ...
    for (let r = 0; r < GRID_SIZE; r++) {
        for (let c = 0; c < GRID_SIZE; c++) {
            ...
            if (value > 0) {
                ...
                // Ładowanie obrazka (poziom wyżej)
...
// Obsługa klawiatury (nasłuchiwanie aktywne tylko gdy gra jest w strukturze dokumentu)
window.addEventListener('keydown', (e) => {
    if (!document.getElementById('g2048-game-container')) return; // Zabezpieczenie przed działaniem w tle
...
// Automatyczny start gry po załadowaniu skryptu
initGame();
